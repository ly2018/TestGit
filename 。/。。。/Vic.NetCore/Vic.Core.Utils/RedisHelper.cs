...
namespace Vic.Core.Utils
{
    ...
    {
        private static string[] ReadWriteHosts = ConfigurationManager.AppSettings.Get("readWriteHosts").Split(new char[] { ';' });
        private static string[] ReadOnlyHosts = ConfigurationManager.AppSettings.Get("readOnlyHosts").Split(new char[] { ';' });
        public RedisHelper()
        {
            Licensing.RegisterLicense(ConfigurationManager.AppSettings.Get("servicestack:license"));
        }
        #region -- 连接信息 --
        ...
        /// <param name="key"></param> 
        /// <param name="t"></param> 
        /// <param name="timeSpan">过期时间分钟</param> 
        /// <returns></returns> 
