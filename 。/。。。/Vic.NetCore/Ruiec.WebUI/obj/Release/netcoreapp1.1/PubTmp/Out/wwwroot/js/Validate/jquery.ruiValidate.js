	...
	rui_validate.prototype = {
        ...
        */
		initForm: function (user_options) {
            ...
            * @time 2015年8月20日 10:07:09
            */
		    rui_Validate_options.FormObj.find("." + rui_Validate_options.SubBtn + " , input[type='submit']").eq(0).click(function (event) {
		    ...
		    /**
             * @content 绑定表单submit事件
             * @author       
             * @time 2015年8月20日 10:07:09
             */
		    rui_Validate_options.FormObj.submit(function (event) {

		        var e = event || window.event;
		        e.stopPropagation();  //阻止事件冒泡 

		        ///var _this = rui_Validate_options.FormObj.find("." + rui_Validate_options.SubBtn + " , input[type='submit']").eq(0);

		        //提交表单进行验证
		        var check_result = _this.Subvalidation(rui_Validate_options, _this);  //获取验证结果
		...
		check_objInfo: function (obj, rui_Validate_options) {
		    ...
		    switch (Verify) {
		        ...
		        case "isIP":
		            var reg = /^[0-9]{0}([0-9]|[.])+$/;
		            if (!reg.test(theValue)) {
		                result = { statue: "Error_validate", obj: obj, msg: "请输入正确的IP地址" };
		            } else {
		                result = { statue: "Correct_validate", obj: obj, msg: "通过验证" };
		            }
		            break;
		        ...
		        case "isPhone":				//是手机号
		            ...
		            break;
		        case "isBankCard":				//是银行卡号
					if(theValue.length < 16 || theValue.length > 19){
						result = { statue: "Error_validate", obj: obj, msg: "您输入的银行卡号格式有误" };
		            } else {
		                result = { statue: "Correct_validate", obj: obj, msg: "通过验证" };
		            }
		            break;
		...
		ShowTips_Bubble: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {  //是否已有提示信息 
		        ...
		        if (state == "Error_validate") {
		            ...
		            var _style = " position: absolute;width:auto; max-width:" + (_obj_width - 15) + ";  background-color:#fff; padding:0px 15px;height:" + (_new_height - 2) + "px;line-height:" + (_new_height - 2) + "px;border:1px solid #ddd; margin-left:" + (_obj_width + 35) + "px;margin-top:-29px;float:left; box-shadow:2px 2px 2px #ddd; background-color:#fff; border-radius:3px;z-index:2;";
		...
		Subvalidation: function (rui_Validate_options, obj) {
		    ...
		    if (_result_cont.length < 1) {
		        ...
		        if (theFormObjUrl != "" && theFormObjUrl != undefined) {  //如果提交是以 AJAX 提交 则执行一下 回调函数 
		            ...
		            // console.log(typeof rui_Validate_options.FormInitData);
		            if (rui_Validate_options.FormInitData != "" && typeof rui_Validate_options.FormInitData == "function") {
		                if (theCollBack == "") {
		                    theCollBack = formSuccess;
		                }

		                var _thePostData = rui_Validate_options.FormInitData(); 
