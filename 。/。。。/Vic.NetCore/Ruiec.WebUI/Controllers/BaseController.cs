    ...
    public abstract class BaseController : Controller
    {
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (_currUser == null)
            {
                ...
                var MobilePhone = User.FindFirst(ClaimTypes.MobilePhone).Value;

                if (string.IsNullOrEmpty(Sid) || string.IsNullOrEmpty(Name) || string.IsNullOrEmpty(MobilePhone))
                {
                    filterContext.Result = new RedirectResult(ConstDefine.AdminLoginUrl);
                    return;
                }
                try
                {
                    _currUser = new UserDto()
                    {
                        ID = new Guid(Sid),
                        UserName = Name,
