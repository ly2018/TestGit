...
namespace Ruiec.WebUI.Controllers
{
    public class ArticleTypeController : BaseController
    {
        ...
        #region 构造器
        public ArticleTypeController(ILanguageTypeAppService languageTypeAppService, IArticleTypeAppService ArticleTypeAppService)
        {
        ...
        {
            PageDataResponse<ArticleTypeDto> pageData = new PageDataResponse<ArticleTypeDto>();
            var parameterExpression = System.Linq.Expressions.Expression.Parameter(typeof(ArticleType));
            ...
            var queryLambdaFilter = System.Linq.Expressions.Expression.Lambda<Func<ArticleType, bool>>(filter, parameterExpression);
            int Count;
            List<ArticleTypeDto> List = _ArticleTypeAppService.PageList(queryLambdaFilter, pageParams.PageSize, pageParams.CurPage, out Count);
            if (List.Any())
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult Create(ArticleTypeDto model)
        {
            ...
            bool IsSucceed = _ArticleTypeAppService.Create(new ArticleTypeDto()
            {
                ID = Guid.NewGuid(),
            ...
            if (IsSucceed)
            {
                return Json(new OperateMessage<string>("/ArticleType/Index"));
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult Edit(ArticleTypeDto model)
        {
            string errs = GetModelStateError();
            if (!string.IsNullOrEmpty(errs))
            {
            ...
            ArticleTypeDto currModel = _ArticleTypeAppService.Get(model.ID);
            if (currModel == null)
            {
            ...
            bool IsSucceed = _ArticleTypeAppService.Edit(currModel);
            if (IsSucceed)
            {
                return Json(new OperateMessage<string>("/ArticleType/Index"));
            }
        ...
        public JsonResult Delete(string Ids)
        {
            ...
            bool IsSucceed = _ArticleTypeAppService.DeleteBatch(delIds);
